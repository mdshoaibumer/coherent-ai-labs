      ...
      <Container>
        <div className="text-center max-w-3xl mx-auto mb-16">
          <Reveal variant="fadeUp">
            ...
            <Text size="lg">
              A resilient, end-to-end architecture processing high-volume inference requests with zero data leakage and complete observability.
            </Text>
          </Reveal>
        </div>

        <Reveal variant="scaleUp">
          <GlassPanel variant="dark" className="p-6 md:p-12 relative overflow-hidden">
            {/* Desktop SVG visualization */}
            <div className="hidden md:block w-full" style={{ aspectRatio: "860/380" }}>
              <svg viewBox="0 0 860 380" className="w-full h-full" role="img" aria-label="Platform architecture diagram showing data flow between system components">
                    ...
                    <g key={`${conn.from}-${conn.to}`}>
                      ...
                      {!prefersReducedMotion && (
                        <m.path
                          ...
                          transition={{ duration: 3 + i * 0.3, delay: i * 0.4, repeat: Infinity, ease: "linear" }}
                        />
                      )}
                    </g>
                  );
            ...
            <div className="md:hidden grid grid-cols-2 gap-3">
              {NODES.map((node) => (
                <div key={node.id} className="p-3 rounded-lg bg-[#0a0a0a] border border-white/10 text-center">
                  <span className="text-xs font-mono text-[#ccc]">{node.label}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-8 pt-6 border-t border-white/5">
              {[
                ...
                <div key={item.label} className="flex items-center gap-2 text-xs font-mono text-[#666]">
